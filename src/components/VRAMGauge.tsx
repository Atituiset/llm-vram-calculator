  ...
  return (
    <div id={COMPONENT_IDS.VRAM_GAUGE} className="bg-slate-900 text-white rounded-2xl p-6 shadow-md flex flex-col gap-6 border border-slate-800">
      ...
      <div className="flex justify-between items-start gap-4">
        <div>
          <span className="text-xs font-bold tracking-wider text-indigo-400 bg-indigo-950/80 border border-indigo-800 px-2 py-0.5 rounded flex items-center gap-1">
          ...
          <div className="text-4xl font-extrabold font-mono text-slate-100 mt-2 flex items-baseline gap-1 animate-pulse">
            {total.toFixed(2)} <span className="text-xl font-semibold text-slate-100">GB</span>
          </div>
        </div>

        {/* Capacity compare indicator */}
        <div className="flex flex-col items-end shrink-0">
          <span className="text-sm text-slate-100">适配典型容量 ({gpuCapacity}GB)</span>
      ...
      {(selectedModel && selectedPrecision) && (
        <div className="bg-slate-950/60 p-3.5 rounded-xl border border-slate-800/80 grid grid-cols-2 gap-2.5 font-sans">
          <div className="flex flex-col col-span-2 pb-1.5 border-b border-slate-850">
            <span className="text-xs text-slate-100 tracking-wider font-bold flex items-center gap-1">
          ...
          <div className="flex flex-col">
            <span className="text-xs font-medium text-slate-100">评估模型物种</span>
            <span className="text-sm font-semibold text-slate-100 truncate mt-0.5" title={selectedModel.name}>
              {selectedModel.name} <span className="text-sm font-mono text-slate-100">({selectedModel.totalParams}B)</span>
            </span>
          </div>

          <div className="flex flex-col">
            <span className="text-xs font-medium text-slate-100">数据计算精度</span>
          ...
          <div className="flex flex-col">
            <span className="text-xs font-medium text-slate-100">运行算力宿主</span>
            <span className="text-sm font-semibold text-slate-100 mt-0.5 truncate" title={selectedGPU?.name}>
              {gpuCount ?? 1}卡 × {selectedGPU?.name || 'GPU'} <span className="text-sm text-slate-100">({gpuCapacity}GB)</span>
            </span>
          </div>

          {selectedMode === 'inference' ? (
            <div className="flex flex-col">
              <span className="text-xs font-medium text-slate-100">推理并发与长度</span>
              <span className="text-sm font-semibold text-emerald-400 mt-0.5 font-mono">
                Bsz={inferenceConfig?.batchSize || 1} · SeqL={inferenceConfig?.sequenceLength || 4096}
              </span>
            </div>
          ) : (
            <div className="flex flex-col">
              <span className="text-xs font-medium text-slate-100">训练并发与长度</span>
              <span className="text-sm font-semibold text-amber-400 mt-0.5 font-mono">
          ...
          <div className="col-span-2 flex flex-wrap gap-1.5 mt-1 pt-1.5 border-t border-slate-800/40 text-xs">
            {selectedMode === 'inference' && inferenceConfig && (
              <>
                <span className="bg-slate-900/80 px-1.5 py-0.5 rounded border border-slate-800 text-slate-100 font-mono">
                ...
                {inferenceConfig.tensorParallelism > 1 && (
                  <span className="bg-indigo-950/45 border-indigo-900/60 text-indigo-400 px-1.5 py-0.5 rounded border">
            ...
            {selectedMode === 'training' && trainingConfig && (
              <>
                ...
                <span className="bg-indigo-950/45 border-indigo-900/60 text-indigo-400 px-1.5 py-0.5 rounded border animate-pulse">
                  激活重算 Checkpoint: On
                </span>
              </>
            )}
      ...
      <div className="flex flex-col gap-1.5 mt-2">
        ...
        <div className="flex flex-wrap gap-x-4 gap-y-2 text-sm text-slate-100 font-mono pt-1">
      ...
      <div id={COMPONENT_IDS.BREAKDOWN} className="border-t border-slate-800 pt-5 flex flex-col gap-3.5">
        <span className="text-sm font-bold tracking-wider text-slate-100 flex items-center gap-1">
          <Binary className="w-4 h-4 text-slate-100" />
        ...
        <div className="flex justify-between items-center text-sm text-slate-100">
          <div className="flex flex-col">
            <span className="font-medium text-slate-100">静态模型权重 / Model Weights (Frozen & Active States)</span>
            <span className="text-xs text-slate-100 leading-relaxed mt-1">
              存放模型全部骨架层所必需的基本占用。
            </span>
          </div>
        ...
        {selectedMode === 'inference' && (
          <div className="flex justify-between items-center text-sm text-slate-100 border-t border-slate-800/40 pt-3">
            <div className="flex flex-col">
              <span className="font-medium text-slate-100">运行时 KV Cache 缓存 / Context KV Cache</span>
              <span className="text-xs text-slate-100 leading-relaxed mt-1">
        ...
        {selectedMode === 'training' && (
          <>
            ...
            <div className="flex justify-between items-center text-sm text-slate-100 border-t border-slate-800/40 pt-3">
              <div className="flex flex-col">
                <span className="font-medium text-slate-100">梯度保存 / Gradient Space</span>
                <span className="text-xs text-slate-100 leading-relaxed mt-1">
            ...
            <div className="flex justify-between items-center text-sm text-slate-100 border-t border-slate-800/40 pt-3">
              <div className="flex flex-col">
                <span className="font-medium text-slate-100">优化器状态 / Optimizer States</span>
                <span className="text-xs text-slate-100 leading-relaxed mt-1">
        ...
        <div className="flex justify-between items-center text-sm text-slate-100 border-t border-slate-800/40 pt-3">
          <div className="flex flex-col">
            <span className="font-medium text-slate-100">局部前向求导激活空间 / Activation Memory</span>
            <span className="text-xs text-slate-100 leading-relaxed mt-1">
              {selectedMode === 'inference'
        ...
        <div className="flex justify-between items-center text-sm text-slate-100 border-t border-slate-800/40 pt-3">
          <div className="flex flex-col">
            <span className="font-medium text-slate-100">CUDA / PyTorch 驻留开销 / System Reserved Buffer</span>
            <span className="text-xs text-slate-100 leading-relaxed mt-1">
